...
from utils.augmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective, addAnnotation  # , add_Gaussianblur
...
def get_hash(paths):
    ...
    只会在第一次读取数据集的时候,将标签文件处理成一个cache文件,这样,再重新读取数据集的时候,直接搜索数据集中是否存在cache文件即可.
    同时,相对于原始的txt标签文件,cache还可以保存很多的额外信息,比如说,数据集的大小,未找到标签文件对应的图片数量…
    如果使用一个损坏的cache文件存放到数据集中,yolov5会不会读取异常? 答案是不会,因为对于每一个数据集的cache文件中,会生成一个Hash Code,这个code是通过数据集的大小计算得到的,因此在读取cache之前会检查Hash Code是否匹配
    """
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index   图片数量/batch size
        ...
        if self.rect:
            # Sort by aspect ratio 获取图片resize的shape
            s = self.shapes  # wh
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        # Cache dataset labels, check images and read shapes 初始化数据集的数量信息
        x = {}  # dict
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # add gaussian blur
            # img = add_Gaussianblur(img, kernal=(hyp['Gaussian_kernal_w'], hyp['Gaussian_kernal_h']), sigma=hyp['Gaussian_sigma'])
            
            # add mask
            img = addAnnotation(img,beta=hyp['mask']) 
