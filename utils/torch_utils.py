...
def torch_distributed_zero_first(local_rank: int):
    ...
    # 该函数用于处理模型分布式训练时的同步问题，如果当前进程不是主进程，设置一个阻塞栅栏，让此进程处于等待状态，等待所有进程到达栅栏处；当所有进程都到达栅栏处后释放栅栏。
...
def select_device(device='', batch_size=0, newline=True):
    ...
    # 该函数实现选择训练的设备，当参数device不为空时，选择指定的设备，即cpu或gpu；当参数device为空时根据机器情况自动选择设备，优先选择gpu作为训练设备。
